...
> **AR-003, D-003.1 — Clasificación: Mixta.** Las Secciones 1-2 (la FSM `EnFriccion`/`EnAdaptacion`/
> `Recuperandose`) son **Histórico** — el ciclo de vida realmente enviado (Commitment Draft→Active) es
> más simple y no implementa estos estados. **La Sección 3.C ("AI Commands") es Evidencia vigente** —
> preservada explícitamente, no histórica: su regla fue citada como evidencia real por
> `docs/ARCHITECTURE_REMEDIATION/AR-047/ANALISIS.md` para D-047.1. Ver
> `docs/02-domain/CLASSIFICATION_STATUS.md` para el registro completo.
...
- **Ejemplos:** `RegistrarFriccion` (disparado por el _ResilienceEvaluator_ tras 48h de inactividad), `ReconstruirProyeccion` (disparado durante el Replay de eventos).
- **Regla:** Solo se ejecutan si se cumplen las precondiciones inmutables de dominio.

### C. AI Commands (Propuestas de IA)

> **AR-003 — Evidencia vigente, preservada (no histórica).** Citada por AR-047/D-047.1. Ver
> `docs/02-domain/CLASSIFICATION_STATUS.md`.

- **Definición:** Sugerencias estructuradas generadas por los modelos de IA.
- **Regla de Oro:** **La IA nunca emite un Comando de Dominio de forma directa.** Sus sugerencias (ej. adaptar un plan) se presentan al usuario en la UI como una **Propuesta**. Solo si el usuario toca _"Aceptar propuesta"_, se emite el Comando Humano correspondiente (`AprobarPlanAdaptado`).
...
- **Riesgo de Estado Inválido por Eventos Desordenados (Fuera de Orden):** Que en escenarios multi-dispositivo y offline un evento `MicroaccionEjecutada` llegue después de que el compromiso se haya marcado como `EnPausa` en local. _(Mitigación: El Agregado valida la secuencia inmutable de eventos. Si un evento viola las transiciones permitidas por la FSM, se rechaza y se genera un evento de conciliación)._

### 2. 💡 Hipótesis (¿Qué estamos asumiendo sin validar?)

- _Hipótesis 1:_ Asumimos que automatizar la transición `Recuperandose` ──► `Activo` tras 3 días de constancia motiva al usuario sin requerir confirmación manual.
