...
*   **Riesgo de Bucle Infinito de Reintentos en Fallos de Red Parciales:** Que una conexión inestable (Edge/3G lento) cause que el cliente envíe eventos, el servidor los guarde, pero la confirmación HTTP de éxito se pierda en el aire. El cliente reintentaría indefinidamente duplicando datos. *(Mitigación: El Inbox Pattern del servidor garantiza que si un evento con el mismo `eventId` (UUIDv7) ya existe en base de datos, se descarta silenciosamente devolviendo éxito 200, evitando duplicación en el Event Store).*

### 2. 💡 Hipótesis (¿Qué estamos asumiendo sin validar?)
*   *Hipótesis 1:* Asumimos que los conflictos multidispositivo ocurren en menos del 1% de las interacciones normales de los usuarios de Commitment.
