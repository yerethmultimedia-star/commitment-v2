...
> el Event Store como fuente de verdad con replay — la dirección que
> `docs/03-architecture/adr_021_goal_backend_and_domain_history_infrastructure.md` reserva
> explícitamente como válida a más largo plazo, sin exigirla hoy. Se preserva como referencia para esa
> opción, no como implementación actual. Ver `docs/02-domain/CLASSIFICATION_STATUS.md`.
...
- **Regla:** El dominio nunca publica directamente al Event Bus. Los eventos se guardan transaccionalmente en la misma base de datos (`Outbox Table`) y un worker de fondo (_Outbox Processor_) los extrae y distribuye de forma asíncrona pero garantizada (_At-Least-Once Delivery_).
...
| **Error de Upcasting**        | Payload de evento antiguo incompatible. | **Recuperación Asistida/Sistema:** El traductor del _Persistence Adapter_ intercepta el evento e inyecta los valores por defecto del nuevo contrato.           |
