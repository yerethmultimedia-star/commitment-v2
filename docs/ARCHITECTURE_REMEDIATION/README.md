...
- **"La propiedad que un hallazgo reclama puede ya existir de forma estructural en el sistema, simplemente sin reconocer ni formalizar" (añadida al cerrar AR-047, 2026-07-23):** propuesta explícitamente como pregunta a formular dentro de Fase 1 — Evidencia, antes de plantear cualquier solución, no como sustitución ni modificación del ciclo de 9 fases. Tres puntos de dato citados por el usuario: AR-024 (la exclusión mutua `goalId`/`commitmentId` ya operaba en el código, solo faltaba una ADR que la formalizara); AR-030 (el aggregate `Identity` ya existía a nivel de dominio, solo faltaba el módulo de backend); AR-047 (el enforcement estructural de D-047.1 ya existía — `packages/domain` nunca dependió de `@nestjs/cqrs` — solo faltaba reconocerlo como propiedad arquitectónica deliberada). **Todavía no promovida a regla permanente ni a paso obligatorio del ciclo** — la Metodología v1.0 quedó congelada al cerrar AR-054 precisamente para que ninguna hipótesis nueva se incorpore sin pasar por el criterio de 5 puntos de Fase 0; se registra aquí como candidata, aplicable ya en la práctica de cada Fase 1 (el propio usuario la calificó de "hipótesis que merece comprobarse", no de regla), pendiente de evaluación formal en la revisión transversal de 15 ARs. **Nota (revisión de 15 ARs, `REVISION_TRANSVERSAL_15_ARS.md`):** se mantiene como candidata — necesita más repetición y, sobre todo, un caso donde permita anticipar correctamente una solución antes de construirla, no solo explicarla después de encontrarla. **Nota (AR-049, 2026-07-23) — cuarto punto de dato, y el primero en satisfacer esa condición pendiente:** Fase 2A de AR-049 anticipó, antes de leer el código de `synchronization-engine.ts`, que el trabajo pendiente "ya no era estructural, sino definir qué hará ese componente" — y Fase 4B confirmó mediante evidencia objetiva (grep de dependencias, grep direccional en todo `apps/mobile/src`, 12/12 tests) que `SynchronizationEngine`, construido por AR-048 un paso antes en la misma sesión, ya satisfacía exactamente D-049.1, sin requerir ningún código nuevo. **El usuario reformula la hipótesis con mayor precisión tras este caso:** no basta con que exista una estructura — debe demostrarse que (1) expresa exactamente la responsabilidad congelada, (2) no contiene responsabilidades adicionales, y (3) ningún otro componente la comparte (la evidencia direccional por grep responde específicamente a esta tercera condición). El usuario distingue además dos formas de cierre igualmente válidas y exigentes en evidencia: **"convergencia por implementación"** (la propiedad no existe, debe implementarse — AR-045, AR-048) y **"convergencia por verificación"** (la propiedad ya existe, debe demostrarse objetivamente — AR-049), formuladas como una única hipótesis candidata: _"una remediación puede completarse correctamente tanto implementando una propiedad como demostrando, mediante evidencia suficiente, que dicha propiedad ya estaba materializada por remediaciones anteriores."_ Todavía no promovida — el propio usuario fija el criterio: si este comportamiento reaparece antes de la revisión transversal de 25 ARs, pasaría a ser candidato serio a principio metodológico.
- **"La fuente de verdad arquitectónica se desplaza progresivamente de artefactos físicos hacia modelos explícitos" (añadida al cerrar AR-003, 2026-07-23):** el usuario registró explícitamente esto como tendencia observada, no como regla nueva. Cinco pares citados: ADR → decisión arquitectónica (AR-024, la unidad correcta era la decisión, no el documento ADR); documento → conocimiento arquitectónico (AR-003, la unidad correcta es el fragmento de conocimiento, no el archivo); carpeta → clasificación explícita (AR-003, `CLASSIFICATION_STATUS.md` desacopla el estado del conocimiento de dónde vive físicamente); implementación → propiedad arquitectónica (AR-047/AR-050, D-047.1/D-050.1 fijan propiedades, no mecanismos concretos); estructura física → registro de gobernanza (patrón transversal a AR-002/AR-009/AR-003). El caso mixto de `domain_state_machines.md` (Fase 4B de AR-003) se cita como el punto de dato más claro: ni "archivar el documento" ni "mantenerlo activo" habría preservado correctamente que una sola sección (no el documento completo) sigue siendo evidencia vigente — evidencia de que la unidad correcta realmente era el conocimiento, no el archivo. Sin promover a regla — pendiente de más repetición y de evaluación en la revisión transversal de 25 ARs.
- **"Las remediaciones más sólidas no desacoplan únicamente componentes, sino también capacidades cuya incorporación está diferida en el tiempo" (añadida al cerrar AR-048, 2026-07-23):** el usuario distinguió explícitamente dos formas de desacoplamiento observadas en el programa — tecnológico (AR-050: la plataforma de IA se desacopló primero del proveedor LLM) y temporal (AR-048: la arquitectura Offline se desacopló primero de _cuándo_ existirá persistencia canónica y sincronización real, sin asumir que esa capacidad exista hoy). El desacoplamiento temporal es menos visible que el tecnológico, pero evita el mismo riesgo: que una limitación actual (aquí, la ausencia de persistencia canónica) se convierta en una restricción permanente del diseño. Dos puntos de dato (AR-050 como desacoplamiento tecnológico, AR-048 como desacoplamiento temporal) — el usuario explícitamente NO la eleva a principio metodológico todavía; se registra para vigilar si reaparece antes de la revisión transversal de 25 ARs. Ver `AR-048/ANALISIS.md`.
- **"Una Fase 4B puede descubrir una precondición técnica no conocida durante las fases anteriores que impide materializar una decisión ya validada, sin invalidar dicha decisión" (añadida durante la Fase 4B de AR-034, 2026-07-24; formulación refinada por el usuario tras el registro inicial, para quedar más precisa y falsable — no "las Fase 4B descubren dependencias" en general):** al construir el mecanismo elegido en Fase 4A (regla de lint sobre imports de `tamagui`), la validación reveló que los 82 archivos objetivo son, sin excepción, `.tsx`, y que `expo lint` no analiza ningún archivo `.tsx` en el repositorio hoy — una restricción no conocida durante Fase 2 ni Fase 4A. El usuario calificó el hallazgo como "arquitectónicamente legítimo, no un problema de implementación": D-034.1 no se reabrió, y AR-034 quedó formalmente abierta y bloqueada, dependiente de una AR nueva (AR-055, spin-off) cuyo único alcance es resolver esa precondición — dependencia unidireccional (AR-055 → Fase 4B de AR-034), preservando la propiedad de que cada remediación modifica exactamente una dimensión del sistema. Primer punto de dato — a diferencia de la observación sobre tests de AR-022 (que el usuario decidió no registrar en ningún artefacto por tratarse de un caso aislado), aquí el usuario pidió explícitamente documentarlo como hallazgo de la remediación, sin promoverlo todavía a principio metodológico. **Segundo punto de dato, a escala mucho menor (Fase 4B de la propia AR-055, 2026-07-24):** al validar el mecanismo de transición, apareció un comentario `eslint-disable-next-line react-hooks/exhaustive-deps` cuyo plugin nunca estuvo instalado en el repositorio — una precondición no conocida durante la Fase 1/2A/2B/4A de AR-055, que bloqueaba alcanzar exactamente la línea base esperada. A diferencia del primer punto de dato (bloqueo total, requirió una AR nueva), aquí la precondición era proporcionalmente pequeña (1 de 45 hallazgos) y se resolvió dentro del mismo alcance de AR-055, sin abrir una AR adicional ni instalar el plugin — tras verificar con evidencia que no había intención arquitectónica detrás (ausencia total en `package.json`/lockfile/`node_modules`/historial de git), se clasificó como configuración residual y se eliminó junto con D-055.1 aplicándose sin cambios. La hipótesis se sostiene con una precisión adicional: la proporcionalidad de la respuesta (nueva AR vs. resolución acotada dentro de la misma) depende del tamaño de la precondición descubierta, no solo de su existencia. **Cierre del primer caso, de extremo a extremo (AR-034, 2026-07-24):** con AR-055 cerrada, AR-034 reanudó su Fase 4B en la misma sesión, exactamente donde se interrumpió, sin reabrir D-034.1 ni su Fase 4A, y cerró con las 5 validaciones pendientes satisfechas — incluida la que originalmente no pudo ejecutarse (la regla aplicándose sobre `.tsx`). Esto confirma la hipótesis con su primer caso íntegramente resuelto: la precondición se resolvió mediante una AR independiente y, una vez resuelta, la AR original no necesitó ningún ajuste propio — el desacoplamiento entre ambas quedó validado empíricamente, no solo argumentado. Ver `AR-034/ANALISIS.md` y `AR-055/ANALISIS.md`.
